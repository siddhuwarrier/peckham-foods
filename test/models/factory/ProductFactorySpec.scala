...
class ProductFactorySpec extends FunSpec with ShouldMatchers {
  describe("The Product Factory") {
    it("should build a product with the details extracted from an XML entity") {
      val xmlElem = utils.getXmlReader(ProductFactorySpec.VALID_SAMPLE_PRODUCT_FILE)
    ...
    it("should not build a product if the XML entity is empty") {
      val xmlElem = utils.getXmlReader(ProductFactorySpec.EMPTY_SAMPLE_PRODUCT_FILE)
      ProductFactory.build(xmlElem) should be(None)
    }

    it("should not build a product if the name is missing") {
      val xmlElem = utils.getXmlReader(ProductFactorySpec.SAMPLE_PRODUCT_FILE_WITH_NO_NAME)
      ProductFactory.build(xmlElem) should be(None)
    }

    it("should not build a product if the EAN is missing") {
      val xmlElem = utils.getXmlReader(ProductFactorySpec.SAMPLE_PRODUCT_FILE_WITH_NO_EAN)
      ProductFactory.build(xmlElem) should be(None)
    }

    it("should not build a product if the list price is missing") {
      val xmlElem = utils.getXmlReader(ProductFactorySpec.SAMPLE_PRODUCT_FILE_WITH_NO_LIST_PRICE)
      ProductFactory.build(xmlElem) should be(None)

    }

    it("should not build a product if the wholesale price is missing") {
      val xmlElem = utils.getXmlReader(ProductFactorySpec.SAMPLE_PRODUCT_FILE_WITH_NO_WHOLESALE_PRICE)
      ProductFactory.build(xmlElem) should be(None)

    }

    it("should not build a product if list price is not a double") {
      val xmlElem = utils.getXmlReader(ProductFactorySpec.SAMPLE_PRODUCT_FILE_WITH_NON_NUMERIC_LIST_PRICE)
      ProductFactory.build(xmlElem) should be(None)
    }

    it("should not build a product if the list price and wholesale price have different currencies") {
      val xmlElem = utils.getXmlReader(ProductFactorySpec.SAMPLE_PRODUCT_FILE_WITH_DIFFERENT_CURRENCY_ATTRIBUTES)
      ProductFactory.build(xmlElem) should be(None)
    }
    ...
    it("should build a product with the default currency if the list price and wholesale price are missing a currency attribute") {
      running(FakeApplication()) {
        val xmlElem = utils.getXmlReader(ProductFactorySpec.SAMPLE_PRODUCT_FILE_WITH_NO_CURRENCY_ATTRIBUTE)
    ...
    it("should not build a product if the currency attribute is invalid") {
      val xmlElem = utils.getXmlReader(ProductFactorySpec.SAMPLE_PRODUCT_FILE_WITH_INVALID_CURRENCY_ATTRIBUTE)
      ProductFactory.build(xmlElem) should be(None)
    }
